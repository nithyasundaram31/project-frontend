...
//     const [initializing, setInitializing] = useState(true); // camera permission lock
//     const warningLockRef = useRef(false); // debounce lock
...
//         if (initializing) return; // Don't trigger during permission

//         if (warningLockRef.current) return; // debounce prevent multiple warnings at same moment
...
//         setTimeout(() => { warningLockRef.current = false; }, 1500); // 1.5 second debounce
...
//             {examStatus === 'started' && (
//                 <>
//                     <div className="fixed top-10 right-5 bg-white p-4 shadow rounded">
//                         <h2>Time Left: {Math.floor(timeLeft / 60)}:{String(timeLeft % 60).padStart(2, '0')}</h2>
//                         <p>Warnings: {warningCount}/3</p>
//                     </div>
//                     <div className="max-w-3xl mx-auto bg-white p-6 shadow rounded mt-10">
//                         <h1 className="text-2xl font-bold mb-4">{examData.name}</h1>
//                         {questions.map((q, idx) => (
...
//                             </div>
//                         ))}
//                         <button disabled={isSubmitting} onClick={submitExams}
